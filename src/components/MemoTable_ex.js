...
const MemoBoard = () => {
	...
	const text = useSelector(state => state.text.boardText)
	...
	const weekTextBox = useCallback(
		text => {
			const box = text.map(thatWeek => {
				...
				return (
					<div>
						<div style={{ display: 'flex', width: '1350px', backgroundColor: 'green' }}>{memoBox(memo)}</div>
						<hr />
						<br />
					</div>
				)
					...
					{text ? (
						...
						>
							<div style={{ width: '240px', height: '125px', margin: 'auto', paddingTop: '10px', overflow: 'hidden' }}>
								...
								<br />
								<ol style={{ paddingLeft: '20px' }}>{planBox(m['plan'])}</ol>
								<br />
								{etcBox(m['etc'])}
							</div>
						</div>
	...
	const aBox = aList => {
		const box = aList.map(a => {
			return <div>{aList != null ? <p style={{ color: 'orange' }}>- {a}</p> : null}</div>
		})
	...
	const conclusionBox = conclusion => {
		const box = conclusion.map(c => {
			return <div style={{ backgroundColor: 'salmon' }}>=&gt; {c}</div>
		})
	...
	return (
		<div className="MemoTable">
			<div>
				...
				<button
					onClick={() => {
						dispatch(memoAction.setWeekBoxLineUp(0))
					}}
				>
					최근 날짜부터
				</button>
				<button
					onClick={() => {
						dispatch(memoAction.setWeekBoxLineUp(1))
					}}
				>
					오래된 것부터
				</button>
				...
				<button
					onClick={() => {
						dispatch(memoAction.setMemoBoxLineUp(0))
					}}
				>
					순서대로
				</button>
				<button
					onClick={() => {
						dispatch(memoAction.setMemoBoxLineUp(1))
					}}
				>
					캘린더
				</button>{' '}
				...
				<button
					onClick={() => {
						dispatch(memoAction.setMemoBoxReverse(false))
					}}
				>
					원래대로
				</button>
				<button
					onClick={() => {
						dispatch(memoAction.setMemoBoxReverse(true))
					}}
				>
					reverse
				</button>
				...
				<div style={{ width: '1370px', height: '900px', overflow: 'scroll', margin: 'auto' }}>
					{weekText.length == 0 ? <div>???</div> : <div style={weekBoxLineUpStyle()}>{weekTextBox(weekText)}</div>}
				</div>
			</div>

			<ReactModal isOpen={open} onRequestClose={() => setOpen(false)} contentLabel="asdfasdf" ariaHideApp={false}>
				{memo(memoText)}
			</ReactModal>
		</div>
	)
