...
exports.createCurrentText = async function (userId) {
	var createCurrent = await knex('text').insert({
		user_id: userId,
		text_title: 'current',
		text_content: 'hello...?',
...
exports.getTextTitleList = async function (userId) {
	var data = await knex('text')
		.select('text_title', knex.raw("date_format(created_at, '%Y.%m.%d %T') as created_at"), knex.raw("date_format(updated_at, '%Y.%m.%d %T') as updated_at"))
		.where({
			user_id: userId,
		})
...
exports.getTextList = async function (userId, textTitleList) {
	var data = await knex('text')
		.select('text_title', 'text_content', knex.raw("date_format(created_at, '%Y.%m.%d %T') as created_at"), knex.raw("date_format(updated_at, '%Y.%m.%d %T') as updated_at"))
...
exports.deleteTextByTitle = async function (userId, textTitle) {
	var deleteTextByTitle = await knex('text').where('user_id', userId).andWhere('text_title', textTitle).del()

	return deleteTextByTitle
}
